 ...
 */
export function getFriendPairId(uid1, uid2) {
 ...
 */
export async function adminRemoveFriend(uid1, uid2) {
 ...
 */
export async function getUserFriendRequests(uid) {
  const requestsRef = collection(db, 'friendRequests');

  // Incoming
  const qIn = query(requestsRef, where('toUid', '==', uid));
  const snapIn = await getDocs(qIn);
  const incoming = snapIn.docs.map(d => ({ id: d.id, direction: 'incoming', ...d.data() }));

  // Outgoing
  const qOut = query(requestsRef, where('fromUid', '==', uid));
  const snapOut = await getDocs(qOut);
  const outgoing = snapOut.docs.map(d => ({ id: d.id, direction: 'outgoing', ...d.data() }));
