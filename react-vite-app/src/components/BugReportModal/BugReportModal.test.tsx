...
const mockedSubmitBugReport = vi.mocked(submitBugReport);
const mockedGetBugReportsByUser = vi.mocked(getBugReportsByUser);
...
describe('BugReportModal', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    mockedGetBugReportsByUser.mockResolvedValue([]);
  });

  // Helper to get form fields by their id attributes
  const getTitleInput = (): HTMLElement | null => document.getElementById('bug-title');
  const getCategorySelect = (): HTMLElement | null => document.getElementById('bug-category');
  const getDescriptionTextarea = (): HTMLElement | null => document.getElementById('bug-description');
  const getStepsTextarea = (): HTMLElement | null => document.getElementById('bug-steps');
  const getSubmitButton = (): Element | null => document.querySelector('.bug-report-submit');
  ...
  describe('form submission', () => {
    ...
    it('should call submitBugReport when form is valid and submitted', async () => {
      const user = userEvent.setup();
      mockedSubmitBugReport.mockResolvedValueOnce('new-report-id');
      ...
      expect(mockedSubmitBugReport).toHaveBeenCalledTimes(1);
      expect(mockedSubmitBugReport).toHaveBeenCalledWith(
    ...
    it('should show error message on submission failure', async () => {
      const user = userEvent.setup();
      mockedSubmitBugReport.mockRejectedValueOnce(new Error('Rate limit exceeded'));
  ...
  describe('My Reports tab', () => {
    it('should switch to My Reports tab when clicked', async () => {
      ...
      expect(mockedGetBugReportsByUser).toHaveBeenCalledWith('test-uid');
    });

    it('should show empty state when no reports exist', async () => {
      const user = userEvent.setup();
      mockedGetBugReportsByUser.mockResolvedValueOnce([]);
