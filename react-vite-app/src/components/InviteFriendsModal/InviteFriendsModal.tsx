...
import { subscribeToAllPresence, type PresenceMap } from '../../services/presenceService';
...
function InviteFriendsModal({ onClose, lobbyDocId, difficulty }: InviteFriendsModalProps): React.ReactElement {
  ...
  const handleInvite = async (friendUid: string): Promise<void> => {
  ...
  const sortedFriends = [...(friends as Friend[])].sort((a: Friend, b: Friend) => {
  ...
  return createPortal(
    <div className="invite-overlay" onClick={onClose}>
      <div className="invite-modal" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
