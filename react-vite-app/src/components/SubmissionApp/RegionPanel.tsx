...
export interface RegionPanelProps {
  ...
  onRegionUpdate: (id: string, updates: RegionUpdateData) => void
  onRegionDelete: (id: string) => void
  onStartDrawing: (mode?: DrawModeType) => void
...
function RegionPanel({
  ...
  const [showDeletePlayingAreaConfirm, setShowDeletePlayingAreaConfirm] = useState<boolean>(false)
  ...
  const handleFloorToggle = (floor: number): void => {
    setEditFloors(prev => {
  ...
  const handleDeletePlayingArea = (): void => {
  ...
  const formatFloors = (floors: number[] | undefined): string => {
