...
vi.mock('./PolygonDrawer', () => ({
  default: ({ regions, selectedRegionId, isDrawing, drawMode, onRegionSelect, onPointAdd, onPolygonComplete, onPointMove }: {
    ...
    onPointMove: (regionId: string, pointIndex: number, newPos: { x: number; y: number }) => void;
  }) => (
...
vi.mock('./RegionPanel', () => ({
  default: ({
    ...
    selectedRegionId: string | null;
    onRegionSelect: (id: string) => void;
    onRegionUpdate: (id: string, data: Record<string, unknown>) => void;
    onRegionDelete: (id: string) => void;
    onStartDrawing: () => void;
    onCancelDrawing: () => void;
    isDrawing: boolean;
    drawMode: string;
    newPolygonPoints: unknown[];
  }) => (
...
describe('MapEditor', () => {
  ...
  beforeEach(() => {
    ...
    mockOnSnapshot.mockImplementation((queryOrRef: unknown, callback: (snapshot: unknown) => void) => {
      onSnapshotCallCount++;
