...
describe('useGameState', () => {
  ...
  describe('startGame', () => {
    ...
    it('should call getRandomImage', async () => {
      ...
      expect(mockedGetRandomImage).toHaveBeenCalledTimes(1);
    });
    ...
    it('should handle image loading error', async () => {
      mockedGetRandomImage.mockRejectedValue(new Error('Network error'));
  ...
  describe('submitGuess', () => {
    ...
    it('should apply floor penalty when floor matches but building is wrong', async () => {
      const imageInDifferentBuilding: GameImage = {
        ...mockImage,
        correctLocation: { x: 51, y: 50 }, // Building B (near boundary)
        correctFloor: 2
      };
      ...
      act(() => {
        result.current.placeMarker({ x: 49, y: 50 }); // Building A (close, but wrong building)
      });
  ...
  describe('nextRound', () => {
    ...
    it('should load new image', async () => {
      ...
      mockedGetRandomImage.mockResolvedValueOnce(mockImage).mockResolvedValueOnce(newImage);
  ...
  describe('scoring algorithm', () => {
    ...
    it('should use default location if correctLocation is missing', async () => {
      ...
      mockedGetRandomImage.mockResolvedValue(imageWithoutLocation);
    ...
    it('should use null floor if correctFloor is missing', async () => {
      ...
      mockedGetRandomImage.mockResolvedValue(imageWithoutFloor);
