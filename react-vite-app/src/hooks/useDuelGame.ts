  ...
  useEffect(() => {
    ...
    if (hasValidGuess) {
      ...
      submitDuelGuess(lobbyDocId, userUid, {
        ...
        timedOut: true,
        noGuess: false
      }, currentImage, duelState?.roundStartedAt ?? undefined, duelState?.timePenaltyEnabled as boolean | undefined).catch((err: unknown) => console.error('Auto-submit failed:', err));
      ...
      submitDuelGuess(lobbyDocId, userUid, {
        ...
        timedOut: true,
        noGuess: true
      }, currentImage, duelState?.roundStartedAt ?? undefined, duelState?.timePenaltyEnabled as boolean | undefined).catch((err: unknown) => console.error('No-guess submit failed:', err));
      setHasSubmitted(true);
    }
  }, [phase, timeRemaining, localGuessLocation, localGuessFloor, localAvailableFloors, currentImage, lobbyDocId, userUid, duelState?.roundStartedAt, duelState?.timePenaltyEnabled, lobbyRoundTime]);
  ...
  const submitGuess = useCallback(async (): Promise<void> => {
    ...
    try {
      await submitDuelGuess(lobbyDocId, userUid, {
        ...
        timedOut: false,
        noGuess: false
      }, currentImage, duelState?.roundStartedAt ?? undefined, duelState?.timePenaltyEnabled as boolean | undefined);
    } catch (err) {
      console.error('Submit guess failed:', err);
      setHasSubmitted(false); // Allow retry
    }
  }, [hasSubmitted, localGuessLocation, localGuessFloor, localAvailableFloors, currentImage, lobbyDocId, userUid, duelState?.roundStartedAt, duelState?.timePenaltyEnabled]);
